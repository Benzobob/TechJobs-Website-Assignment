...
<script>
	function DeleteJob(x)
      {	
			...
			{
				var str1 = "DeleteJobForUser.php?whid=";
...
<div class="container">
  <div class="row">
	...
	<ul class="list-group">
		...
		$uid = $_SESSION['id'];
		$sql = "SELECT * FROM WorkHistory WHERE UserID = $uid";
		$result = $conn->query($sql);
		echo "<div class='container'><table class='table table-striped'>";
		while($row = $result->fetch_assoc()) {
			echo "<tr>";
			echo "<td>" .  $row['CompanyName'] . " - " . $row['JobDescription'] . "</td>";
			echo "<td><a style='cursor: pointer;' onclick='DeleteJob(" . $row['WHID'] . ")'><i class='material-icons'>delete</i></a></td>";
		}
		echo "</tr>";
		?>
		<br>
